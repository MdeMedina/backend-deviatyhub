...
async function verifyAllEndpointsSync() {
  ...
  try {
    ...
    const validHash = await hashBcrypt('password123');
    (mockPrisma.user.findUnique as any).mockResolvedValueOnce({
    ...
    (mockPrisma.user.create as any).mockResolvedValueOnce({ id: 'u-inv', email: 'new@docs.com', inviteToken: 't-999' });
